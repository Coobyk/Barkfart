...
app.innerHTML = `
  <header class="hero">
    <div class="hero-emoji" aria-hidden="true">ð¶ð¨</div>
    <h1>Barkfart</h1>
    <p>
    ...
    <span class="tag">MIDI â barkfart â MP3</span>
  </header>
    ...
    <div class="options">
      <div class="field">
        ...
        <div class="mode-pills" id="modes">
          ...
          <button type="button" data-mode="bark">All barks ð¶</button>
          <button type="button" data-mode="fart">All farts ð¨</button>
          <button type="button" data-mode="alternate">Alternate</button>
        </div>
      </div>
      <div class="field">
        <label for="splitMidi">Hand split â below = fart, at/above = bark</label>
  ...
  <section class="card">
    ...
    <div class="transport">
      <div class="viz" id="viz" aria-hidden="true">
        ${Array.from({ length: 88 }, (_, i) => `<span data-midi=\"${i + 21}\"></span>`).join('')}
      </div>
      <input type="range" class="seek" id="seek" min="0" max="1000" value="0" disabled />
      <div class="transport-row">
        <button type="button" id="play" disabled>â¶ Play</button>
        <button type="button" class="secondary" id="stop" disabled>â  Stop</button>
  ...
  <section class="card">
    ...
    <div class="controls" style="margin-top: 0.85rem">
      <button type="button" id="download" disabled>â¬ Download MP3</button>
      <button type="button" class="secondary" id="downloadWav" disabled>â¬ Download WAV</button>
    </div>
  ...
  <footer>
    <p class="about">
      ...
      An internet meme remix style where music is rebuilt with dog barks and fart samples â
...
function updateVisualizer(t) {
  const levels = new Float32Array(88)
  if (player.playing) for (const note of player.notes) { const i = note.midi - PIANO_LOW_MIDI; if (t >= note.time && t <= note.time + note.duration && i >= 0 && i < 88) levels[i] = Math.max(levels[i], note.velocity) }
  vizBars.forEach((bar, i) => { bar.style.height = `${Math.max(4, levels[i] * 100)}%`; bar.classList.toggle('active', levels[i] > 0) })
}
...
player.onProgress = (t, dur) => { updateTimeUI(t, dur); updateVisualizer(t) }
player.onEnded = () => {
...
async function loadFile(file) {
  ...
  if (!/\.midi?$/i.test(name) && file.type && !file.type.includes('midi')) {
    // still try â some systems omit type
  }
  ...
  try {
    ...
    metaEl.innerHTML = `
      ...
      <div>${meta.noteCount} notes Â· ${meta.tracks} tracks Â· ~${meta.bpm.toFixed(0)} BPM Â· ${formatTime(meta.duration)}</div>
    `
...
async function runExport(format) {
  ...
  exportStatus.textContent = 'Mixing barkfartâ¦'
  ...
  try {
    ...
    const samples = await player.renderForExport((pct) => {
      ...
      exportStatus.textContent = `Mixingâ¦ ${Math.round(pct)}%`
    })

    let blob
    let filename
    if (format === 'wav') {
      exportStatus.textContent = 'Writing WAVâ¦'
      setExportProgress(96)
      blob = encodeWav(samples, EXPORT_SAMPLE_RATE)
      filename = `${fileBaseName}-barkfart.wav`
      setExportProgress(100)
    } else {
      exportStatus.textContent = 'Encoding MP3â¦'
      setExportProgress(92)
      try {
        blob = await encodeMp3(samples, EXPORT_SAMPLE_RATE, (pct) => {
          setExportProgress(pct)
          exportStatus.textContent = `Encoding MP3â¦ ${Math.round(pct)}%`
        })
        ...
        filename = `${fileBaseName}-barkfart.wav`
        exportStatus.className = 'export-status busy'
        exportStatus.textContent = 'MP3 failed â downloading WAV insteadâ¦'
      }
    }

    downloadBlob(blob, filename)
    exportStatus.className = 'export-status done'
    exportStatus.textContent = `Done â ${filename}`
    setExportProgress(100)
  } catch (e) {
    console.error(e)
